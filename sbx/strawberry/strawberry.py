...
from sbx.common.type_aliases import BatchNormTrainState, ReplayBufferSamplesNp, RLBatchNormTrainState
from sbx.crossq.crossq import ConstantEntropyCoef, CrossQ, EntropyCoef
from sbx.strawberry.policies import StrawberryPolicy


class Strawberry(CrossQ):
    policy_aliases: ClassVar[Dict[str, Type[StrawberryPolicy]]] = {
        "MlpPolicy": StrawberryPolicy,
        "MultiInputPolicy": StrawberryPolicy,
    }

    policy: StrawberryPolicy
    ...
    def __init__(
        ...
        OffPolicyAlgorithmJax.__init__(
            self,
            policy=policy,
            env=env,
    ...
    def update_critic(
        ...
        qf_state: RLBatchNormTrainState,
        ...
        key, noise_key, dropout_key, dropout_key_target = jax.random.split(key, 4)
        ...
        def mse_loss(
            ...
            current_q_values, _ = jnp.split(q_values, 2, axis=1)

            qf_next_values = qf_state.apply_fn(
                {"params": qf_state.target_params, "batch_stats": qf_state.batch_stats},
                next_observations,
                next_state_actions,
        ...
        return qf_state, (qf_loss_value, ent_coef_value), key

    @staticmethod
    @jax.jit
    def soft_update(tau: float, qf_state: RLBatchNormTrainState) -> RLBatchNormTrainState:
